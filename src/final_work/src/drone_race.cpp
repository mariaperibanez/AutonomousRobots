...
DroneRace::DroneRace(ros::NodeHandle nh) : nh_(nh)
{
    ...
    gate_update_timer_ = nh_.createTimer(
        ...
        &DroneRace::gateUpdateTimerCallback_, this);
...
void DroneRace::gateUpdateTimerCallback_(const ros::TimerEvent& event) {
    // Update the gates with noise
    updateGateObservations_();
    // Draw the new gates
    drawGates_();
    // Generate a new trajectory with the new gates
    generateTrajectory_();
}
